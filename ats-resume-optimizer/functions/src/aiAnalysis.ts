...
interface MatchAnalysis {
    matchedSkills: Array<{ skill: string; importance: string; confidence: number }>;
...
interface GapAnalysis {
    criticalGaps: Array<{ skill: string; importance: string; hasTransferable: boolean }>;
    minorGaps: Array<{ skill: string; importance: string; hasTransferable: boolean }>;
 ...
 */
function calculateATSScore(matchAnalysis: MatchAnalysis): number {
    const weights = {
        matchedSkills: 0.5,
        keywordDensity: 0.2,
        experienceMatch: 0.2,
 ...
 */
function determineReadiness(atsScore: number, gaps: GapAnalysis): boolean {
 ...
 */
export const performGapAnalysis = onCall(
    {
        region: "us-central1",
        timeoutSeconds: 120,
        memory: "512MiB",
    ...
    async (request) => {
        ...
        if (!taskId || !resume || !job) {
            throw new HttpsError(
                "invalid-argument",
                "taskId, resume, and job are required."
            );
        }
...
Provide the analysis JSON.`.trim();
            ...
            let analysisResult: { matchAnalysis: MatchAnalysis; gaps: GapAnalysis };
            ...
            throw new HttpsError(
                "internal",
                error.message || "Failed to perform gap analysis."
            );
        }
    }
);
...
export const generateRecommendation = onCall(
    ...
    async (request) => {
        ...
        if (!resume || !job || !gaps) {
            throw new HttpsError(
                "invalid-argument",
                "resume, job, and gaps are required."
            );
        }
...
async function generateUpskillPath(
    openai: OpenAI,
    perplexityKey: string,
    resume: any,
    job: any,
...
Candidate's Current Skills: ${resume.skills?.map((s: any) => s.name).join(", ") || "Not specified"}
...
async function findAlternativeJobs(
    openai: OpenAI,
    perplexityKey: string,
    resume: any,
    job: any
...
Resume Skills: ${resume.skills?.map((s: any) => s.name).join(", ") || "Not specified"}
 ...
 */
export const optimizeResume = onCall(
    {
        region: "us-central1",
        timeoutSeconds: 180,
        memory: "1GiB",
    ...
    async (request) => {
        ...
        if (!taskId || !resume || !job || !analysis) {
            throw new HttpsError(
                "invalid-argument",
                "taskId, resume, job, and analysis are required."
            );
        }
...
Return JSON:
{
  ...
  "changes": [
    ...
    { "type": "experience_improvement", "reason": "..." }
            ...
            let result: { optimizedResume: any; changes: any[] };
 ...
 */
export const addSkillToResume = onCall(
    {
        region: "us-central1",
        timeoutSeconds: 120,
        memory: "512MiB",
    ...
    async (request) => {
        ...
        if (!taskId || !resume || !skill || !targetSections) {
            throw new HttpsError(
                "invalid-argument",
                "taskId, resume, skill, and targetSections are required."
            );
        }
 ...
 */
export const generatePrepGuide = onCall(
    {
        region: "us-central1",
        timeoutSeconds: 300, // 5 minutes for all sections
        memory: "1GiB",
        secrets: [openaiApiKey, perplexityApiKey],
    },
    async (request) => {
        ...
        if (!applicationId || !companyName || !jobTitle) {
            throw new HttpsError(
                "invalid-argument",
                "applicationId, companyName, and jobTitle are required."
            );
        }
        ...
        try {
            ...
            const callGpt = async (prompt: string, taskName: string): Promise<string> => {
                try {
                    const response = await openai.chat.completions.create({
                        model: "gpt-4o-mini",
                        messages: [
                    ...
                    const content = response.choices[0]?.message?.content;
                    if (!content) throw new Error(`No content for ${taskName}`);
                    return content.trim();
                } catch (error: any) {
...
Include: Pre-interview checklist, day-of routine, virtual/onsite tips, mindset advice.`;
            ...
            throw new HttpsError("internal", error.message || "Failed to generate prep guide.");
        }
    }
);
...
export const generateCoverLetter = onCall(
    ...
    async (request) => {
        ...
        if (!applicationId || !resume || !jobTitle || !company) {
            throw new HttpsError(
                "invalid-argument",
                "applicationId, resume, jobTitle, and company are required."
            );
        }
...
Please output ONLY the text of the cover letter, starting with the header. Do not include markdown naming blocks or introductory conversational text.
            ...
            throw new HttpsError("internal", error.message || "Failed to generate cover letter.");
        }
    }
);
 ...
 */
export const onBackgroundTaskCreated = onDocumentCreated(
    {
...
async function processOptimizeResume(
    task: any,
  ...
  "changes": [{ "type": "...", "reason": "..." }]
    ...
    if (historyId || analysis.id) {
        ...
        await historyRef.update({
            ...
            draftMatchAnalysis: analysis.matchAnalysis, // unchanged for pure optimization
            baselineAtsScore: calibratedScore,  // Store baseline for skill addition calculations
            baselineTotalSkills: totalSkillsNeeded,  // Store total skills count at baseline
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
...
async function processAddSkill(
    task: any,
    taskRef: admin.firestore.DocumentReference,
    openai: OpenAI,
    db: admin.firestore.Firestore
...
Return a JSON object with 'optimizedResume' and 'changes'.
    ...
    const targetExperienceIds: string[] = [];
    const genericSections: string[] = [];

    targetSections.forEach((section: string) => {
    ...
    if (targetExperienceIds.length > 0 && resume.experience) {
        targetExperienceIds.forEach(expId => {
            const exp = resume.experience.find((e: any) => e.id === expId);
            if (exp) {
    ...
    const experienceTargets = targetExperienceIds.length > 0
        ? targetExperienceIds.map(id => {
            const exp = resume.experience?.find((e: any) => e.id === id);
...
- "reason": Brief description of what was changed
    ...
    addedSkills.forEach((skillName: string) => {
        ...
        if (!updatedMatchAnalysis.matchedSkills.find((m: any) => m.skill.toLowerCase() === lowerSkill)) {
        ...
        updatedMatchAnalysis.partialMatches = updatedMatchAnalysis.partialMatches.filter(
            (p: any) => p.skill.toLowerCase() !== lowerSkill
        );
        updatedMatchAnalysis.missingSkills = updatedMatchAnalysis.missingSkills.filter(
            (m: any) => m.skill.toLowerCase() !== lowerSkill
        );
    });
...
async function processPrepGuide(
    task: any,
    taskRef: admin.firestore.DocumentReference,
    openai: OpenAI,
    db: admin.firestore.Firestore
    ...
    sections.companyIntelligence = await callPerplexity(
        ...
        `Research ${companyName} for interview preparation for a ${jobTitle} role.`
    );
 ...
 */
export const generateTrainingSlideshow = onCall(
    {
        region: "us-central1",
        timeoutSeconds: 300,
        memory: "512MiB",
