...
interface GapAnalysis {
  criticalGaps: Array<{ skill: string; importance: string; hasTransferable: boolean; impactOnApplication?: string }>;
...
export const performGapAnalysis = (openaiApiKey: any, perplexityApiKey: any) => onCall(
  {
    region: "us-central1",
    timeoutSeconds: 120,
    memory: "512MiB",
  ...
  async (request) => {
    if (!request.auth) {
      throw new HttpsError("unauthenticated", "You must be logged in to perform analysis.");
    }

    const { taskId, resume, job } = request.data;
    if (!taskId || !resume || !job) {
      throw new HttpsError("invalid-argument", "taskId, resume, and job are required.");
    }
...
8. analysisMetadata.analysisComplete is true only if all validations pass.`.trim();
      ...
      const matchAnalysis: MatchAnalysis = {
        ...
        experienceMatch: { match: analysisResult.overallScore || 0 }, // Simplified fallback
        readinessVerdict: (analysisResult.readinessLevel || 'stretch').toLowerCase() as any,
        verdictSummary: analysisResult.executiveSummary || ""
      };

      const gaps: GapAnalysis = {
        criticalGaps: (analysisResult.matchAnalysis?.noMatches || [])
          .filter((m: any) => m.importance === 'critical' || m.importance === 'high')
          .map((m: any) => ({
            skill: m.requirement || m.skill,
      ...
      throw new HttpsError("internal", error.message || "Failed to perform gap analysis.");
    }
  }
);
