  ...
  "changes": [
    { "type": "REWRITE_BULLET", "section": "Experience", "reason": "Quantified achievement to increase ATS impact" }
            ...
            const optimizedResume = {
                ...
                ...aiGeneratedResume, // Overwrite with AI result
                contactInfo: (resume.contactInfo && Object.keys(resume.contactInfo).length > 0)
                    ? resume.contactInfo
                    : (aiGeneratedResume?.contactInfo || {}), // Prioritize original contact info if present
                experience: (resume.experience || []).map((origExp: any, idx: number) => {
