...
import { openRouterApiKey, blotatoApiKey } from "./secrets";

/**
 * Helper to standardise CORS across internal backend endpoints
 */
function applyCors(req: functionsV1.Request, res: functionsV1.Response) {
 ...
 */
export const generateMarketingIdeas = functionsV1
    .region("us-central1")
    .runWith({ secrets: [openRouterApiKey], timeoutSeconds: 90 })
 ...
 */
export const modifyTone = functionsV1
    .region("us-central1")
    .runWith({ secrets: [openRouterApiKey], timeoutSeconds: 30 })
    .https.onRequest(async (req, res) => {
        if (applyCors(req, res)) { res.status(204).send(''); return; }
        ...
        try {
            ...
            const response = await axios.post("https://openrouter.ai/api/v1/chat/completions", {
                model: "openai/gpt-4o-mini", // fast and cheap for text mapping
                messages: [{ role: "user", content: prompt }]
            }, {
                headers: { "Authorization": `Bearer ${openRouterApiKey.value().trim()}`, "Content-Type": "application/json" }
            });
 ...
 */
export const generateMediaPrompts = functionsV1
    .region("us-central1")
    .runWith({ secrets: [openRouterApiKey], timeoutSeconds: 30 })
    .https.onRequest(async (req, res) => {
        if (applyCors(req, res)) { res.status(204).send(''); return; }
 ...
 */
export const generateBlogPost = functionsV1
    .region("us-central1")
    .runWith({ secrets: [openRouterApiKey], timeoutSeconds: 300 })
    .https.onRequest(async (req, res) => {
        if (applyCors(req, res)) { res.status(204).send(''); return; }
        ...
        try {
            ...
            const response = await axios.post("https://openrouter.ai/api/v1/chat/completions", {
                model: "openai/gpt-4o", // using fully powered 4o for large format blog generation
                messages: [{ role: "user", content: prompt }]
            }, {
                headers: { "Authorization": `Bearer ${openRouterApiKey.value().trim()}`, "Content-Type": "application/json" }
            });
 ...
 */
function getBlotatoTarget(platform: string) {
    const map: Record<string, { accountId: number, targetType: string, pageId?: string }> = {
 ...
 */
export const publishToBlotato = functionsV1
    .region("us-central1")
    .runWith({ secrets: [blotatoApiKey] })
    .https.onRequest(async (req, res) => {
        if (applyCors(req, res)) { res.status(204).send(''); return; }
        ...
        try {
            ...
            const mediaUrlArray = urls.map((u: any) => u.value).filter((val: string) => val.trim() !== "");
            ...
            const publishPromises = platforms.map(async (plat: string) => {
                const targetDetails = getBlotatoTarget(plat);
                if (!targetDetails) return { platform: plat, success: false, error: "No account mapping found for this platform." };
...
Web: https://www.riresume.com`;
                ...
                try {
                    const response = await axios.post("https://backend.blotato.com/v2/posts", payload, {
                        headers: {
                            "Authorization": `Bearer ${blotatoApiKey.value().trim()}`,
                            "Content-Type": "application/json"
                        }
                    });
