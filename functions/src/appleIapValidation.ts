 ...
 */
const PRODUCT_TOKEN_MAP: Record<string, { tokens: number; packageId: string; price: number }> = {
 ...
 */
export const validateAppleReceipt = functionsV1
    .region("us-central1")
    ...
    .https.onCall(async (data: any, context: any) => {
        ...
        if (!context.auth) {
            throw new functionsV1.https.HttpsError(
                "unauthenticated",
                "You must be logged in to validate a purchase."
            );
        }
        ...
        try {
            ...
            batch.set(activityRef, {
                ...
                contextData: {
                    packageId: tokenInfo.packageId,
                    tokens: tokenInfo.tokens,
                    amount: tokenInfo.price, // Added price for UI display
                    transactionId,
                    source: "apple_iap",
                },
                platform: "ios",
            ...
            throw new functionsV1.https.HttpsError(
                "internal",
                error.message || "Failed to validate Apple receipt."
            );
        }
    });
...
async function validateWithApple(
    receiptData: string,
    sharedSecret: string
): Promise<{ valid: boolean; status: number; receipt?: any; latest_receipt_info?: any[] }> {
