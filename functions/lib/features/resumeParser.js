...
const parseResume = (openaiApiKey) => functionsV2.https.onCall({
...
- Return EXACT JSON, no explanations.`.trim();
            ...
            if (!contentResponse || response.choices[0].finish_reason === 'length') {
                ...
                const refusal = message?.refusal || 'None';
                console.error(`System returned empty/truncated content. Finish Reason: ${reason}. Refusal: ${refusal}`);
                let userReason = reason;
                if (reason === 'content_filter')
                    userReason = "Content policy violation (safety)";
                if (reason === 'length')
                    userReason = "The document is too long to parse. Please shorten your resume.";
...
function _addIdsToParsedData(parsed, originalText) {
    const generateId = () => `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    return {
        ...parsed,
        experience: (parsed.experience || []).map((exp) => ({ ...exp, id: generateId() })),
        education: (parsed.education || []).map((edu) => ({ ...edu, id: generateId() })),
        certifications: (parsed.certifications || []).map((cert) => ({ ...cert, id: generateId() })),
        projects: (parsed.projects || []).map((proj) => ({ ...proj, id: generateId() })),
